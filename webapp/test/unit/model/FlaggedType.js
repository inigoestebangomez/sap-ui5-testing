...
sap.ui.define([
	...
	QUnit.test("Should convert 1 to true", function (assert) {
		// Act
		let bFormattedValue = new FlaggedType().formatValue(1);
		// Assert
	...
	QUnit.test("Should convert other values to false", function (assert) {
		let oFlaggedType = new FlaggedType();

		// Act
		let bFormattedZero = oFlaggedType.formatValue(0);
		let bFormattedNegativeNumber = oFlaggedType.formatValue(-666);
		// Assert
	...
	QUnit.test("Should parse false to 0", function (assert) {
		// Act
		let iParsedValue = new FlaggedType().parseValue(false);
		// Assert
	...
	QUnit.test("Should parse true to 1", function (assert) {
		// Act
		let iParsedValue = new FlaggedType().parseValue(true);
		// Assert
